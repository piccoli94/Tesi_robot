...
% beta,sigma interaction region defining constants
% w_s saturation term
xe=forward_kinematics(q);                                 %forward kinematics
Delta_x=xe-xdes;                                          %position error
region=dominant_region(Delta_x);                          %region evaluaion
[w,A]=weight_vector(Delta_x, xdes, region, a_h, a_r, a_t);% weighted vector and transition matrix A
x_d=x_i+w*(xdes-x_i);                                     % weighted trajectory
delta_x=xe-x_d;                                           % weighted position error
k_p=pos_stiffness(delta_x, region, k_1, a, a_h)           % position dependent stiffness 
J=jacobian(q);                                            % Jacobian   
...
Y_d=regres_matrix(q, dq, dqr, ddqr);                      %regressor matrix
...
c_x=interaction_tem(interact_region, beta, sigma, f_h, s_x); %interaction term 
