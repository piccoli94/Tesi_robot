...
pci= sym('pci',[3,1,noJoints],'real'); pci(1,1,:)=0;
...
for i=1:7
    for j=1:7
        for k=1:7
            hlp(i,j)=hlp(i,j)+diff(Blp(i,j),q(k))-0.5*diff(Blp(j,k),q(i));
            hlo(i,j)=hlo(i,j)+diff(Blo(i,j),q(k))-0.5*diff(Blo(j,k),q(i));
            hmp(i,j)=hmp(i,j)+diff(Bmp(i,j),q(k))-0.5*diff(Bmp(j,k),q(i));
            hmo(i,j)=hmo(i,j)+diff(Bmo(i,j),q(k))-0.5*diff(Bmo(j,k),q(i));
        end
    end
end